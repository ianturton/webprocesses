...
public class PickerProcess extends StaticMethodsProcessFactory {
	...
	static public SimpleFeatureCollection getFeatures(
			...
			@DescribeParameter(name = "organisation", description = "The name of the organisation making the request", min = 0) String organisation) {
		Filter f = parseFilterString(filter);
		return getFeatures(collection, f);
	}

	static public SimpleFeatureCollection getFeatures(SimpleFeatureCollection collection, Filter filter) {
		...
		if (filter == null) {
			...
			ret = collection.subCollection(filter);
	 ...
	 */
	protected static Filter parseFilterString(String filter) {
		Filter ret = null;
